...
func runSearch(api *jira.Client, parsedQuery *parsedQuery, jql string, maxResults int) {
	var query string

	if parsedQuery != nil && jql == "" {
		query = buildJQL(parsedQuery)
	} else if jql != "" && parsedQuery == nil {
		query = jql
	} else {
		return
	}

	issues, err := getIssues(api, query, maxResults)
	if err != nil {
        errMsg := fmt.Sprintf("%s\n\n**Press ⏎ to go back**", err.Error())
		wf.NewItem(err.Error()).
			Subtitle("Press ⏎ to show full error").
			Icon(aw.IconError).
			Arg(errMsg).
			Var("action", "error").
			Valid(true)
